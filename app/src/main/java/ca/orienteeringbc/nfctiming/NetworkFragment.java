...
public class NetworkFragment extends Fragment {
    ...
    private class DownloadTask extends AsyncTask<String, Integer, DownloadTask.Result> {
        ...
        protected Result doInBackground(String... urls) {
            ...
            if (mUrlString.contains("events")) {
                // Determine the event number, save as {event_number}_event.xml
                int end = mUrlString.lastIndexOf('/') - 1;
                int start = mUrlString.lastIndexOf('/', end) + 1;
                mFileName = mUrlString.substring(start, end) + "_event.xml";
            } else {
                mFileName = "clubs.xml";
            }
        ...
        private String downloadUrl(URL url) throws IOException {
            ...
            try {
                ...
                // Retrieve file length (could be -1)
                int fileLength = connection.getContentLength();

                // Retrieve the response body as an InputStream.
                stream = connection.getInputStream();
                output = getActivity().openFileOutput(mFileName, 0);
                publishProgress(DownloadCallback.Progress.GET_INPUT_STREAM_SUCCESS, 0);

                if (stream == null) {
                    publishProgress(DownloadCallback.Progress.ERROR, 0);
                } else {
                    ...
                    while ((count = stream.read(data)) != -1) {
                        ...
                        if (fileLength > 0) // only if total length is known
                            publishProgress(DownloadCallback.Progress.PROCESS_INPUT_STREAM_IN_PROGRESS, (int) (total * 100 / fileLength));
